...
public class AuthController {
    ...
    /**
     * Отправка шестизначного кода подтверждения на email для регистрации.
     * Пользователь вводит этот код на странице регистрации для подтверждения email.
     */
    @PostMapping("/send-verification-code")
    public ResponseEntity<String> sendVerificationCode(@RequestParam String email) {
        log.debug("Отправка кода подтверждения на email: {}", email);
     ...
     * После успешной проверки кода создаётся новый пользователь.
     */
    ...
    public ResponseEntity<AuthResponse> verifyAndRegister(
        ...
        log.debug("Проверка кода и регистрация для email: {}", request.getEmail());
        AuthResponse response = authService.verifyCodeAndRegister(request, code);
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }
}